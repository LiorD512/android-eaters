...
import com.bupp.wood_spoon_eaters.model.AppSettingKnownTypes
...
internal class AppSettingAdapter {
    ...
    fun fromJson(appSetting: AppSetting): AppSetting {
        val convertedValue = try {
            when (AppSettingKnownTypes.values().firstOrNull { it.name == appSetting.data_type }) {
            ...
            Timber.e("AppSetting parsing error: data_type=[%s] value=[%s]", appSetting.data_type, appSetting.value)
            appSetting.value
        }
