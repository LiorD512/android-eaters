...
class UserImageView : FrameLayout {
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(context, attrs, defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            if(a.hasValue(R.styleable.UserImageView_isEditButtonVisible)) {
                val isEditBtnVisible = a.getBoolean(R.styleable.UserImageView_isEditButtonVisible, false)
                if(isEditBtnVisible){
                    userImageViewBadge.visibility = View.VISIBLE
                }else{
                    userImageViewBadge.visibility = View.GONE
                }
            }
            a.recycle()
        }
    ...
    private fun setImageSize(imageSize: Int) {
        when (imageSize) {
            Constants.SMALL_IMAGE_SIZE -> {
                ...
                userImageViewLayout.layoutParams = frame

                val imageLayout = LayoutParams(61.toPx(), 61.toPx())
                userImageViewPic.layoutParams = imageLayout

                val badgeLayout = LayoutParams(25.toPx(), 25.toPx())
                badgeLayout.gravity = Gravity.BOTTOM + Gravity.END

                userImageViewBadge.layoutParams = badgeLayout
            }
            else -> {
                val frame = LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT)
                frame.gravity = Gravity.CENTER_HORIZONTAL
                ...
                val badgeLayout = LayoutParams(23.toPx(), 23.toPx())
                badgeLayout.gravity = Gravity.BOTTOM + Gravity.END
    ...
    fun setImage(imageStr: String) {
        Glide.with(context).load(imageStr).apply(RequestOptions.circleCropTransform()).into(userImageViewPic)
    }

    fun setImage(imageUri: Uri) {
        Glide.with(context).load(imageUri).apply(RequestOptions.circleCropTransform()).into(userImageViewPic)
    }
}